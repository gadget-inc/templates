import { FindSimilarMoviesGlobalActionContext } from "gadget-server";

/**
 * @param { FindSimilarMoviesGlobalActionContext } context
 */
export async function run({ params, logger, api, connections, scope }) {
  ...
  const response = await connections.openai.embeddings.create({ input: quote, model: "text-embedding-ada-002" });
  ...
  // remove duplicates
  const filteredMovies = movies.filter((movie, index) => movies.findIndex((m) => m.title === movie.title) === index);
