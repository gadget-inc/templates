import { useFindFirst, useFindMany, useActionForm, Controller } from "@gadgetinc/react";
  ...
  return (
    <Form onSubmit={submit}>
      <FormLayout>
        ...
        {error && (
          <Banner title="Error saving selection" tone="critical">{error.message}</Banner>
        )}
        {formState?.isLoading ?
...
const ShopPage = () => {
  ...
  const [{ data: products, fetching: productsFetching, error: productsFetchError }] = useFindMany(api.shopifyProduct);
  ...
  return (
    <Page
      title="Select product for pre-purchase offer"
    >
      {(productsFetching || shopFetching || productOptions.length === 0) ? (
        ...
        <Layout>
          {(productsFetchError || shopFetchError) && (
            <Layout.Section>
              <Banner title="Error loading data" tone="critical">{productsFetchError?.message || shopFetchError?.message}</Banner>
            </Layout.Section>
          )}
          <Layout.Section>
