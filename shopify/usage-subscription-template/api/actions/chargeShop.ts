...
export const run: ActionRun = async ({ params, logger, api, connections }) => {
  ...
  const shopify = await connections.shopify.forShopId(shop?.id as string);
  ...
  const { amountUsedInPeriod } = await api.shopifyShop.findOne(
    shop?.id as string,
    {
      select: {
  ...
  const subscription = await api.shopifyAppSubscription.maybeFindOne(
    shop?.activeSubscriptionId as string,
    {
      select: {
        lineItems: true,
      },
  ...
  if (amountUsedInPeriod ?? 0 >= cappedAmount) {
    ...
    return await api.internal.shopifyShop.update(shop?.id as string, {
      overage: price,
    });
  }
  ...
  if (remainder)
    await api.internal.shopifyShop.update(shop?.id as string, {
      overage: remainder,
    });
};
