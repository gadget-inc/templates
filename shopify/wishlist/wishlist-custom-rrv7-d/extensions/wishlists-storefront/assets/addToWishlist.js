  ...
  btn.onclick = () => {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      const wishlistResponse = await fetch(
        `${window.shopURL}/apps/wishlist-p-rrv7-d/wishlist/create`,
        {
          method: "POST",
          headers: {
...
function applyCheckmarks(value) {
  ...
  for (const { id, variants } of Object.values(window.wishlistObj)) {
...
async function handleWishlistClick(e) {
  ...
  if (current) {
    ...
    const deleteResponse = await fetch(
      `${window.shopURL}/apps/wishlist-p-rrv7-d/wishlist/item/remove`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const addResponse = await fetch(
      `${window.shopURL}/apps/wishlist-p-rrv7-d/wishlist/item/create`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
